#!/usr/bin/env python3
"""Verified remote lock/unlock for Turo trips on a MyBMW (ConnectedDrive) vehicle.

Sibling of pathfinder_remote.py — identical Turo-trip verification and CLI
contract, but the door backend is ~/bmw-remote (bmw_api) instead of nissan_api.

Before touching the doors it VERIFIES, from the Turo reservation page, that:
  1. the thread/reservation is for a configured vehicle (the BMW), and
...
Then it calls bmw_api (bimmer_connected) to act, confirms via the remote-service
ExecutionState + a lock-state re-read, and prints a JSON result.

bmw_api lives in its own venv (bimmer_connected has dependencies), so unlike the
nissan path this shells out to ~/bmw-remote/.venv/bin/python, not sys.executable.

Usage:
  python3 scripts/bmw_remote.py check  <thread_id>          # verify only, no action
  python3 scripts/bmw_remote.py status <thread_id>          # verify car, read-only telemetry
  python3 scripts/bmw_remote.py unlock <thread_id>
  python3 scripts/bmw_remote.py lock   <thread_id>
...
BMW_DIR = os.path.expanduser("~/bmw-remote")
# bmw_api needs bimmer_connected, which isn't in the Turo venv — use its own.
BMW_PY = os.path.join(BMW_DIR, ".venv", "bin", "python")

# vehicle-name substring (lowercase) -> bmw-remote alias (in its config.json)
VEHICLES = {"bmw 3 series": "bmw3"}
...
def _bmw(args):
    if not os.path.exists(BMW_PY):
        return 1, {"ok": False, "error": "bmw-remote venv not found at %s "
                   "(run: cd %s && python3 -m venv .venv && .venv/bin/pip install -e .)"
                   % (BMW_PY, BMW_DIR)}
    out = subprocess.run(
        [BMW_PY, "-m", "bmw_api", "--json"] + args,
        cwd=BMW_DIR, capture_output=True, text=True,
    )
    try:
...
def _lock_status(alias):
    rc, data = _bmw(["status", alias])
    # bmw_api status JSON: {"ok":true,"vin":...,"status":{"lockStatus":"locked",...}}
    if rc == 0 and isinstance(data, dict):
        return ((data.get("status") or {}).get("lockStatus"))
    return None
...
def main():
    ...
    rc, door = _bmw([action, alias])
    result["door"] = door
    # bmw_api's door result already includes a self-verified lockStatus_after;
    # fall back to a separate status read only if it's absent.
    lock_after = door.get("lockStatus_after") if isinstance(door, dict) else None
    result["lockStatus_after"] = lock_after if lock_after is not None else _lock_status(alias)
